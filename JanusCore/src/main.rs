...
use janus_logwindow_nucleus::LogWindowHandle;
...
async fn main() {
    ...
    // Lecture de la configuration depuis janus_config_nucleus
    ...
    let initial_progress_bar_enabled = config.music_progress_bar.unwrap_or(false);
    ...
    let (_stream, stream_handle) = match PlayerService::initialize_audio() {
        Ok(s) => s,
        Err(e) => {
            janus_log_nucleus::log_error(format!("Impossible d'initialiser l'audio : {}", e));
            return;
        }
    };
